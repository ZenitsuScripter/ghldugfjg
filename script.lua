...
local function verificarAutenticacao()
    ...
    local success, senhaGitHub = pcall(function()
        return game:HttpGet("https://raw.githubusercontent.com/ZenitsuScripter/SDF/main/hhhhjjjjj" .. antiCache, true)
    end)
    
    if not success then
        game.Players.LocalPlayer:Kick("❌ Erro de conexão com servidor de autenticação.\n\nTente novamente mais tarde.\n\nErro: " .. tostring(senhaGitHub))
        return false
    end
    
    -- DEBUG: Mostra o que foi baixado (REMOVA DEPOIS)
    print("=== DEBUG SENHA ===")
    print("Tamanho senha GitHub:", #senhaGitHub)
    print("Primeiros 50 chars:", senhaGitHub:sub(1, 50))
    
    -- Remove TODOS caracteres invisíveis
    local senhaLimpa = ""
    for i = 1, #senhaGitHub do
        local char = senhaGitHub:sub(i, i)
        local byte = string.byte(char)
        -- Mantém apenas caracteres visíveis (ASCII 33-126)
        if byte >= 33 and byte <= 126 then
            senhaLimpa = senhaLimpa .. char
        end
    end
    
    print("Tamanho após limpeza:", #senhaLimpa)
    print("Primeiros 50 chars limpos:", senhaLimpa:sub(1, 50))
    
    -- Senha hardcoded no script
    ...
    print("Senhas são iguais?", senhaLimpa == senhaLocal)
    print("===================")
